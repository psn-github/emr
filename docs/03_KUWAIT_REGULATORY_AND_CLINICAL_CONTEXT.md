...
1. **Married couples only.** ART is permitted only within a valid marriage between husband and wife. The clinical unit is the **couple**, and a **verified marriage certificate is a hard gate** before any fertility workflow can begin. This is why the `Couple` and `MarriageVerification` entities are first-class in the data model and why cycle creation is blocked without a verified marriage record.
