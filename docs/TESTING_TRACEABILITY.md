...
3. **Non-functional harness (baseline).** ⚠️ **PARTLY DONE.** The audit-append concurrency baseline is proven (`audit/…/concurrency.integration.test.ts`, ADR-0061 — it caught + fixed the missing retry: ~48/50 concurrent appends previously failed). HTTP k6 load (`perf/k6-api-load.js`) is a ready template but **deferred** until the tRPC HTTP host is mounted (ADR-0009 — `apps/api` is a composition root today). See `perf/README.md`.
