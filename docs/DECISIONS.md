...
## ADR-0070 — Self-bootstrapping staging deploy; API binds loopback
- **Status:** accepted (2026-07-04)
- **Context:** PR #97 merged and the first auto-deploy failed with `missing server host` — the `VPS_HOST`/`VPS_USER`/`VPS_SSH_KEY` repo secrets are unset. GitHub Actions secrets are write-only, so the om-software repo's existing `VPS_SSH_PRIVATE_KEY` cannot be read or copied (and this build session has no SSH path to the VPS — port 22 is blocked by the sandbox network policy). The remaining manual setup should be as small as possible, and it must stay impossible to disturb the om-software stack that shares the VPS.
- **Decision:** (1) **Self-bootstrap:** the deploy workflow, on a fresh VPS, clones the public repo to `/opt/oxford-his` and runs `scripts/vps-bootstrap.sh` — idempotent one-time setup (Node 20 only if absent, pnpm via corepack, PostgreSQL only if nothing serves :5432, `oxford_staging` DB + role with a generated password written once to `/etc/oxford-his/api.env`, systemd unit, nightly-backup cron). Every step checks before acting; re-runs are no-ops; the script touches only the oxford-his paths/unit/DB/cron — never `/opt/oxmedkw` or the om-software nginx sites. **Adding the three repo secrets is therefore the ONLY manual step** (a fresh dedicated deploy key; the om-software key cannot be extracted). (2) **Loopback bind:** `serve.ts` binds `127.0.0.1` by default (`BIND` env to override) — the staging API must not sit on a public interface while it authenticates dev bearer tokens; reach it via SSH tunnel or the optional TLS nginx site. Document store moves to `/opt/oxford-his-data` (outside the deploy path, like the DB).
- **Consequences:** once the secrets exist, the very next deploy run stands up the whole staging stack unattended and every later run is a normal additive deploy; the dev-token surface is never internet-exposed by default; om-software isolation is enforced by construction in the bootstrap.

## ADR-0071 — Staging deploys unattended: no approval gate on the `staging` environment
- **Status:** accepted (2026-08-07, product owner)
- **Context:** ADR-0064 described the deploy as approval-gated on the `staging` GitHub environment. With the VPS secrets added (2026-08-07), the product owner chose not to configure a required reviewer: staging holds synthetic data only (ADR-0007), the deploy pipeline is additive with the destructive-migration gate, and an approval click on every merge adds friction without protecting anything clinical.
