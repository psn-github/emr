...
import { ageingBuckets, revenueByLine, instalmentRisk, revenueLeakage, cycleDisposition, type AgeingItem, type AgeingBuckets, type RevenueRow, type InstalmentRisk, type LeakageCharge, type LeakageReport, type DispositionCounts, type DispositionReport } from "./dashboards.js";
...
export class AnalyticsService {
  ...
  /** Revenue-cycle: leakage from billable, uninvoiced charges (with aged subset). */
  leakage(charges: readonly LeakageCharge[], agedOverDays?: number): LeakageReport {
    return revenueLeakage(charges, agedOverDays);
  }
